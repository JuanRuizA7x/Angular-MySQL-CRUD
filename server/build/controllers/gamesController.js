...
class GamesController {
    ...
    updateGame(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const id = Number(req.params.id);
            database_1.default.query(`UPDATE games set ? WHERE id = ${id}`, [req.body], function (err, game, fields) {
                if (err)
                    res.json({ error: err });
    ...
    deleteGame(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const id = Number(req.params.id);
            yield database_1.default.query(`DELETE FROM games WHERE  id =${id}`, function (err, game, fields) {
                if (err)
                    res.json({ error: err });
