...
class GamesController {
    ...
    public async createGame (req: Request, res: Response): Promise<void> {
        ...
        await pool.query('INSERT INTO games set ?', [req.body], function(err, game, fields) {
            if (err) res.json({error: err});
    ...
    public async updateGame (req: Request, res: Response) {
        const id = Number(req.params.id);
        pool.query(`UPDATE games set ? WHERE id = ${id}`, [req.body], function(err, game, fields) {
            if (err) res.json({error: err});
    ...
    public async deleteGame (req: Request, res: Response) {
        const id = Number(req.params.id);
        await pool.query(`DELETE FROM games WHERE  id =${id}`, function(err, game, fields) {
            if (err) res.json({error: err});
